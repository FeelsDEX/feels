...
// WORKAROUND: Anchor 0.31.1 Account Parsing Issues
//
// This file provides a workaround for account deserialization bugs in Anchor 0.31.1.
// The issue manifests when creating Program instances with IDLs that have complex
// account structures, causing "account size mismatch" errors.
//
// Root cause: Anchor 0.31.1 has breaking changes in how it handles account definitions
// and type references in IDL files, particularly with nested structures.
//
// Workaround strategy:
// 1. Strip account definitions from IDL (use only instructions and types)
// 2. Normalize type references to ensure consistent format
// 3. Manually add account fetch methods if needed
//
// TODO: Monitor Anchor releases and remove this workaround when fixed upstream
// Tracking: https://github.com/coral-xyz/anchor/issues (search for account parsing)
//
// Last updated: Oct 2024 (Anchor 0.31.1)
...
export function createFeelsProgram(provider: AnchorProvider): Program {
  try {
    // Get program ID from environment variable or fallback to hardcoded
    const programId = process.env['NEXT_PUBLIC_FEELS_PROGRAM_ID'] || 'B3w6rjs2vDjr6eKUXUiERV44BXud3nRqshctMp5p4au4';
    
    console.log('Program ID from env or fallback:', programId);
    console.log('Type of programId:', typeof programId);
    console.log('programId length:', programId?.length);
    
    // Validate the program ID before using it
    try {
      new PublicKey(programId);
      console.log('Program ID validation passed');
    } catch (pubkeyError) {
      console.error('Program ID validation failed:', pubkeyError);
      throw new Error(`Invalid program ID: ${programId}. Please check NEXT_PUBLIC_FEELS_PROGRAM_ID environment variable.`);
    }
    
    // First, try with the full IDL as-is, using correct constructor signature
    // Program constructor: new Program(idl, provider) - program ID comes from IDL
    try {
      console.log('Attempting to create program with full IDL and programId:', programId);
      
      // Create a modified IDL with the correct program ID
      const idlWithProgramId = {
        ...FEELS_IDL,
        address: programId
      } as Idl;
      
      const program = new Program(idlWithProgramId, provider);
    ...
    // Create program with modified IDL, setting the address in the IDL
    console.log('Creating program with modified IDL and programId:', programId);
    const modifiedIdlWithAddress = {
      ...modifiedIDL,
      address: programId
    } as Idl;
    const program = new Program(modifiedIdlWithAddress, provider);
