...
/// Market field data needed for work calculation
#[derive(Clone, Debug)]
...
pub struct PathSegment {
    /// Starting reserves
    pub reserve_a_start: u128,
    pub reserve_b_start: u128,
    
    /// Ending reserves
    pub reserve_a_end: u128,
    pub reserve_b_end: u128,
...
pub fn calculate_path_work(params: &PathWorkParams) -> Result<WorkResult, String> {
    ...
    let (w_hat_s, w_hat_t, w_hat_l) = get_hat_weights(&params.field);
    
    // Calculate work for each segment
    for segment in &params.segments {
        let work = calculate_segment_work(segment, &params.field, w_hat_s, w_hat_t, w_hat_l)?;
        segment_work.push(work);
        total_work = total_work.saturating_add(work);
    }
    
    // Map work to fee
    let estimated_fee = map_work_to_fee(total_work, &params.field)?;
    
    // Calculate max rebate (capped by tau)
    let max_rebate = calculate_max_rebate(total_work, &params.field)?;
    ...
    match segment.dimension {
        TradeDimension::Spot => {
            ...
            let S_start = calculate_spot_value(
                segment.reserve_a_start,
                segment.reserve_b_start,
                field.twap_a,
                field.twap_b,
                field.sigma_price,
            )?;
            ...
            let S_end = calculate_spot_value(
                segment.reserve_a_end,
                segment.reserve_b_end,
                field.twap_a,
                field.twap_b,
                field.sigma_price,
            )?;
...
fn calculate_spot_value(
    ...
    // Value in numeraire
    let value_a = (reserve_a * twap_a) / Q64;
    let value_b = (reserve_b * twap_b) / Q64;
    
    // Geometric mean (for equal weights)
    let spot_value = sqrt_u128(value_a * value_b)?;
...
mod tests {
    ...
    fn test_spot_work_calculation() {
        ...
        let segment = PathSegment {
            reserve_a_start: 1000 * Q64,
            reserve_b_start: 1000 * Q64,
            reserve_a_end: 900 * Q64,
            reserve_b_end: 1111 * Q64,
