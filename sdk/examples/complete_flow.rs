...
use feels_sdk::{
    derive_pool, find_buffer_address, find_market_address, sort_tokens, FeelsClient, HubRouter,
    PoolInfo, SdkConfig,
};
use solana_sdk::{
    pubkey::Pubkey,
...
async fn main() -> anyhow::Result<()> {
    ...
    let config = SdkConfig::localnet(payer).with_commitment("confirmed".to_string());
    ...
    let (pool_address, pool_bump) =
        derive_pool(&token_0, &token_1, fee_rate, &client.config.program_id);
    println!(
        "  Pool PDA: {} (bump: {}) @ {}bps",
        pool_address, pool_bump, fee_rate
    );
    ...
    println!(
        "    Output: {} SOL",
        quote.amount_out as f64 / 1_000_000_000.0
    );
    println!(
        "    Fee: {} USDC ({}bps)",
