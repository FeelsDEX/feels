...
test_devnet!(
    ...
    |ctx: TestContext| async move {
        ...
        println!(
            "User entered FeelsSOL system with {} FeelsSOL",
            feelssol_balance
        );
        ...
        match ctx
            .exit_feelssol(&user, &user_feelssol, &user_jitosol, exit_amount_1)
            .await
        {
            Ok(_) => {
            ...
            Err(e) => {
                ...
                if e.to_string().contains("OracleStale") || e.to_string().contains("6009") {
                    ...
                    println!(
                        "  This is a limitation of the in-memory test environment Clock sysvar"
                    );
                } else {
        ...
        let (protocol_config_pubkey, _) =
            Pubkey::find_program_address(&[b"protocol_config"], &feels::id());
        let protocol_config = ctx
            .get_account::<feels::state::ProtocolConfig>(&protocol_config_pubkey)
            .await?
            .ok_or("Protocol config not found")?;

        println!(
            "  DEX TWAP stale age: {} seconds",
            protocol_config.dex_twap_stale_age_secs
        );

        // On devnet, we can't manipulate time directly, but we can demonstrate the safety check
        // by examining the oracle state
        let (protocol_oracle_pubkey, _) =
            Pubkey::find_program_address(&[b"protocol_oracle"], &feels::id());

        let oracle = ctx
            .get_account::<ProtocolOracle>(&protocol_oracle_pubkey)
            .await?
        ...
        println!(
            "    Native last update timestamp: {}",
            oracle.native_last_update_ts
        );
        println!(
            "    DEX last update timestamp: {}",
            oracle.dex_last_update_ts
        );

        // Check safety controller state
        let (safety_controller_pubkey, _) =
            Pubkey::find_program_address(&[b"safety_controller"], &feels::id());

        let safety = ctx
            .get_account::<SafetyController>(&safety_controller_pubkey)
            .await?
        ...
        println!(
            "    Depeg threshold: {} bps",
            protocol_config.depeg_threshold_bps
        );
        println!(
            "    Required observations for pause: {}",
            protocol_config.depeg_required_obs
        );
        println!(
            "    Required observations for resume: {}",
            protocol_config.clear_required_obs
        );
        ...
        println!(
            "    High volatility: {}",
            safety.degrade_flags.high_volatility
        );
        ...
        println!(
            "WARNING: Note: Full oracle freshness testing requires devnet/mainnet environment"
        );
...
test_devnet!(
    test_oracle_staleness_over_time,
    |ctx: TestContext| async move {
        ...
        .await?;
        ctx.enter_feelssol(&user, &user_jitosol, &user_feelssol, 2_000_000_000)
            .await?;
...
test_all_environments!(
    test_safety_controller_pause_resume,
    |ctx: TestContext| async move {
        ...
        println!(
            "  Depeg threshold: {}% ({} bps)",
            config.depeg_threshold_bps as f64 / 100.0,
            config.depeg_threshold_bps
        );
        println!(
            "  Required breaches to pause: {}",
            config.depeg_required_obs
        );
        println!("  Required clears to resume: {}", config.clear_required_obs);
        println!("  DEX TWAP window: {} seconds", config.dex_twap_window_secs);
        println!(
            "  DEX TWAP stale threshold: {} seconds",
            config.dex_twap_stale_age_secs
        );
...
test_all_environments!(test_rate_limit_enforcement, |ctx: TestContext| async move {
    ...
    println!(
        "  Mint per slot cap: {} FeelsSOL",
        config.mint_per_slot_cap_feelssol
    );
    println!(
        "  Redeem per slot cap: {} FeelsSOL",
        config.redeem_per_slot_cap_feelssol
    );
    ...
    if config.redeem_per_slot_cap_feelssol > 0 {
        ...
        for (i, user) in users.iter().enumerate() {
            let user_feelssol = ctx
                .get_or_create_ata(&user.pubkey(), &ctx.feelssol_mint)
                .await?;
            let user_jitosol = ctx
                .get_or_create_ata(&user.pubkey(), &ctx.jitosol_mint)
                .await?;

            match ctx
                .exit_feelssol(user, &user_feelssol, &user_jitosol, amount_per_user)
                .await
            {
                Ok(_) => {
                    successful_exits += 1;
...
test_all_environments!(
    test_oracle_update_permissions,
    |ctx: TestContext| async move {
        println!("\n=== Test: Oracle Update Permissions ===");
        ...
        let ix = sdk_compat::update_native_rate(unauthorized.pubkey(), native_rate_q64);
        ...
        let ix = sdk_compat::update_native_rate(payer.pubkey(), native_rate_q64);
