...
mod token_validate_integration_tests {
    ...
    fn test_create_token_with_restricted_ticker() {
        ...
        let restricted_tickers = vec!["SOL", "FEELSSOL", "USDC", "USDT", "BTC", "ETH"];
        ...
        for ticker in restricted_tickers {
            ...
            assert!(
                result.is_err(),
                "Should reject restricted ticker: {}",
                ticker
            );
            ...
            assert!(
                error_msg.contains("RestrictedTicker"),
                "Should be RestrictedTicker error"
            );
            ...
            assert!(
                ...
                "Should provide alternatives for: {}",
                ticker
            );
    ...
    fn test_create_token_with_valid_ticker() {
        ...
        let valid_tickers = vec!["MYTOKEN", "CUSTOMCOIN", "DEFI123", "MOON", "LAMBO", "TEST1"];
        ...
        for ticker in valid_tickers {
            ...
            assert!(
                !is_ticker_restricted(ticker),
                "Valid ticker should not be restricted: {}",
                ticker
            );
        }
    }
    ...
    fn test_ticker_case_insensitive_restriction() {
        ...
        for (ticker, should_be_restricted) in test_cases {
            ...
            assert_eq!(
                is_restricted, should_be_restricted,
                "Ticker '{}' restriction status should be: {}",
                ticker, should_be_restricted
            );
        }
    }
    ...
    fn test_token_metadata_structure() {
        ...
        assert!(
            TokenMetadata::SIZE >= expected_min_size,
    ...
    fn test_complete_token_create_validation_workflow() {
        ...
        println!(
            "Alternatives for '{}': {:?}",
            restricted_ticker, alternatives
        );
        ...
        assert!(
            valid_result.is_ok(),
            "Valid ticker '{}' should pass validation",
            valid_ticker
        );
    ...
    fn test_error_message_clarity() {
        ...
        assert!(
            error_string.contains("restricted"),
        ...
        assert!(
            format_string.contains("characters") || format_string.contains("format"),
    ...
    fn test_comprehensive_restriction_coverage() {
        ...
        for ticker in required_tickers {
            assert!(
                is_ticker_restricted(ticker),
                "Required ticker '{}' should be restricted",
                ticker
            );
        }
        ...
        for ticker in protocol_tokens
            .iter()
            .chain(&blockchain_tokens)
            .chain(&stablecoins)
            .chain(&reserved_words)
        {
            assert!(
                is_ticker_restricted(ticker),
                "Category ticker '{}' should be restricted",
                ticker
            );
        }
    ...
    fn test_restrict_lookup_performance() {
        ...
        let test_tickers = vec!["SOL", "USDC", "BTC", "ETH", "MYTOKEN", "CUSTOM", "TEST123"];
        ...
        println!(
            "1000 iterations x {} tickers took: {:?}",
            test_tickers.len(),
            duration
        );
        assert!(
            duration.as_millis() < 100,
            "Restriction lookups should be fast"
        );
    }
}
