...
test_all_environments!(
    ...
    |ctx: TestContext| async move {
        ...
        match ctx.enter_feelssol(&user, &user_jitosol, &user_feelssol, jitosol_amount).await {
            Ok(_) => {},
            Err(e) => {
    ...
    |ctx: TestContext| async move {
        ...
        match ctx.get_account_raw(&safety_controller).await {
            ...
            Err(e) => {
                ...
                eprintln!("  This may indicate protocol initialization didn't create safety controller");
            }
        }
