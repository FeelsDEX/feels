...
test_in_memory!(test_can_create_test_context, |ctx: TestContext| async move {
...
test_in_memory!(test_can_airdrop_sol, |ctx: TestContext| async move {
    ...
    // Airdrop 1 SOL
    ctx.airdrop(&recipient.pubkey(), constants::LAMPORTS_PER_SOL).await?;
    
    // Check balance using the client
    let balance = ctx.client.lock().await
...
test_all_environments!(test_create_and_use_token, |ctx: TestContext| async move {
    ...
    let mint = ctx.create_mint(&ctx.accounts.market_creator.pubkey(), 9).await?;
    
    // Create associated token account for alice
    let alice_token_account = ctx.create_ata(&ctx.accounts.alice.pubkey(), &mint.pubkey()).await?;
...
test_in_memory!(test_pre_configured_accounts, |ctx: TestContext| async move {
    ...
    for account in accounts {
        ...
        assert!(account_data.is_some(), "Test account {} should exist", account.pubkey());
    }
