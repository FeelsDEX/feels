//! Test using System Program as dummy accounts
use crate::common::*;
...
test_in_memory!(test_system_dummy_initialize_market, |ctx: TestContext| async move {
    println!("\n=== System Dummy Initialize Market Test ===");
    ...
    // Protocol token PDAs - use the actual PDAs or rent sysvar as dummy
    let (protocol_token_0, protocol_token_0_readonly) = if token_0 == ctx.feelssol_mint {
        // Use rent sysvar as dummy for FeelsSOL (it's a valid account that exists)
        (solana_sdk::sysvar::rent::id(), true)
    } else {
        // Use the actual protocol token PDA
        let (pda, _) = Pubkey::find_program_address(&[b"protocol_token", token_0.as_ref()], &PROGRAM_ID);
        (pda, false)
    };
    
    let (protocol_token_1, protocol_token_1_readonly) = if token_1 == ctx.feelssol_mint {
        // Use clock sysvar as dummy for FeelsSOL (different from rent to avoid duplication)
        (solana_sdk::sysvar::clock::id(), true)
    } else {
        // Use the actual protocol token PDA
        let (pda, _) = Pubkey::find_program_address(&[b"protocol_token", token_1.as_ref()], &PROGRAM_ID);
        (pda, false)
    };
    ...
    let accounts = vec![
        ...
        if protocol_token_0_readonly {
            AccountMeta::new_readonly(protocol_token_0, false)
        } else {
            AccountMeta::new(protocol_token_0, false)
        },                                                      // 10: protocol_token_0
        if protocol_token_1_readonly {
            AccountMeta::new_readonly(protocol_token_1, false)
        } else {
            AccountMeta::new(protocol_token_1, false)
        },                                                      // 11: protocol_token_1
        AccountMeta::new(escrow, false),                        // 12: escrow
        AccountMeta::new_readonly(solana_sdk::sysvar::instructions::id(), false), // 13: dummy creator_feelssol
        AccountMeta::new_readonly(solana_sdk::sysvar::slot_hashes::id(), false), // 14: dummy creator_token_out
    ...
    for (i, account) in accounts.iter().enumerate() {
        println!("  {}: {} (writable: {})", i, account.pubkey, account.is_writable);
    }
