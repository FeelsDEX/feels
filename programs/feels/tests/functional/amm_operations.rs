...
mod amm_functional_tests {
    ...
    fn test_liquidity_position_calculation() {
        ...
        println!("Desired amounts: {}, {}", amount_0_desired, amount_1_desired);
    ...
    fn test_feelssol_routing_concept() {
        ...
        let token_b = "TokenB";
        println!("Two-hop swap route: {} -> {} -> {}", token_a, feelssol, token_b);
    ...
    fn test_slippage_protection_mechanisms() {
        ...
        assert!(good_execution >= minimum_amount_out, "Good execution should pass");
        assert!(bad_execution < minimum_amount_out, "Bad execution should fail");
    ...
    fn test_tick_position_management() {
        ...
        let out_of_range = out_of_range_tick >= position_lower && out_of_range_tick <= position_upper;
    ...
    fn test_complete_user_flow() {
        ...
        println!("✓ Deposited {} SOL, received {} FeelsSOL", sol_deposited, feelssol_received);
        
        // Step 2: Add liquidity
        let feelssol_liquidity = 5.0;
        let usdc_liquidity = 500.0; // $100 SOL price
        println!("✓ Added liquidity: {} FeelsSOL + {} USDC", feelssol_liquidity, usdc_liquidity);
        ...
        println!("✓ Removed liquidity: {} FeelsSOL + {} USDC", removed_feelssol, removed_usdc);
        
        // Step 6: FeelsSOL -> SOL
        let feelssol_redeemed = feelssol_received + earned_fees;
        let sol_received = feelssol_redeemed; // 1:1 ratio
        println!("✓ Redeemed {} FeelsSOL for {} SOL", feelssol_redeemed, sol_received);
    ...
    fn test_amm_error_conditions() {
        ...
        assert!(large_swap > available_liquidity, "Should detect insufficient liquidity");
        ...
        assert!(invalid_lower > invalid_upper, "Should detect invalid tick range");
    ...
    fn test_amm_efficiency_metrics() {
        ...
        assert!(capital_efficiency >= 50, "Should have reasonable capital efficiency");
        ...
        assert!(fee_efficiency > 0, "Should generate fees for liquidity providers");
    ...
    fn test_slippage_protection_properties() {
        ...
        let min_out = (u128::from(expected_out) * (10000 - u128::from(slippage_tolerance)) / 10000) as u64;
    ...
    fn test_price_impact_calculation_properties() {
        ...
        assert!(price_impact > 1.9 && price_impact < 2.1, "Price impact should be ~2% for 1% sqrt price change");
    }
    ...
    fn test_front_running_mitigation_properties() {
        ...
        assert!(large_trade_size > small_trade_size * 100, "Large trades should be significantly larger");
    ...
    fn test_pool_state_consistency_properties() {
        ...
        assert!(new_liquidity > initial_liquidity, "Liquidity should increase");
    ...
    fn test_transaction_atomicity_properties() {
        ...
        assert!(failed_swap_amount > initial_balance, "Should fail due to insufficient balance");
        ...
        assert_eq!(post_failure_balance, initial_balance, "Balance unchanged after failure");
        assert_eq!(post_failure_liquidity, initial_liquidity, "Liquidity unchanged after failure");
    }
    ...
    fn test_tick_array_router_consistency() {
        ...
        for i in 0..MAX_ROUTER_ARRAYS {
            let is_active = (router.active_bitmap & (1 << i)) != 0;
            if is_active {
                assert_ne!(router.tick_arrays[i], Pubkey::default(), "Active arrays should have valid pubkeys");
            } else {
                assert_eq!(router.tick_arrays[i], Pubkey::default(), "Inactive arrays should be default");
            }
        }
    }
