...
mod tests {
    ...
    fn test_tick_layout() {
        ...
        assert_eq!(
            mem::size_of::<Tick>(),
            80,
            "Tick size must be exactly 80 bytes"
        );
        ...
        unsafe {
            ...
            let fee_growth_0_offset =
                &tick.fee_growth_outside_0_x64 as *const _ as usize - base_ptr;
            let fee_growth_1_offset =
                &tick.fee_growth_outside_1_x64 as *const _ as usize - base_ptr;
            ...
            assert_eq!(
                liquidity_net_offset, 0,
                "liquidity_net should be at offset 0"
            );
            assert_eq!(
                liquidity_gross_offset, 16,
                "liquidity_gross should be at offset 16"
            );
            assert_eq!(
                fee_growth_0_offset, 32,
                "fee_growth_outside_0_x64 should be at offset 32"
            );
            assert_eq!(
                fee_growth_1_offset, 48,
                "fee_growth_outside_1_x64 should be at offset 48"
            );
        ...
        assert_eq!(
            mem::size_of_val(&Tick::default().fee_growth_outside_0_x64),
            16
        );
        assert_eq!(
            mem::size_of_val(&Tick::default().fee_growth_outside_1_x64),
            16
        );
    ...
    fn test_tick_array_layout() {
        ...
        unsafe {
            ...
            assert_eq!(
                start_tick_offset, 32,
                "start_tick_index should be at offset 32"
            );
            assert_eq!(pad0_offset, 36, "_pad0 should be at offset 36");
            assert_eq!(ticks_offset, 48, "ticks array should be at offset 48");
            assert_eq!(
                count_offset,
                48 + (80 * TICK_ARRAY_SIZE),
                "initialized_tick_count should be at offset {}",
                48 + (80 * TICK_ARRAY_SIZE)
            );
            assert_eq!(
                pad1_offset,
                48 + (80 * TICK_ARRAY_SIZE) + 2,
                "_pad1 should be at offset {}",
                48 + (80 * TICK_ARRAY_SIZE) + 2
            );
            assert_eq!(
                reserved_offset,
                48 + (80 * TICK_ARRAY_SIZE) + 2 + 14,
                "_reserved should be at offset {}",
                48 + (80 * TICK_ARRAY_SIZE) + 2 + 14
            );
        }
        ...
        assert_eq!(
            mem::size_of_val(&array.ticks),
            80 * TICK_ARRAY_SIZE,
            "Ticks array should be {} bytes",
            80 * TICK_ARRAY_SIZE
        );
    }
    ...
    fn test_alignment() {
        ...
        assert_eq!(
            mem::align_of::<Tick>(),
            16,
            "Tick should be 16-byte aligned"
        );

        // TickArray should also maintain proper alignment
        assert!(
            mem::align_of::<TickArray>() >= 8,
            "TickArray should be at least 8-byte aligned"
        );
    }
