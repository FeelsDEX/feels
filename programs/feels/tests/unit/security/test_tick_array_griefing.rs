...
test_in_memory!(
    test_tick_array_limit_calculation,
    |ctx: TestContext| async move {
        ...
        for spacing in spacings {
            ...
            println!(
                "Tick spacing {}: {} ticks coverage ≈ {:.1}% price range",
...
test_in_memory!(
    test_griefing_attack_scenarios,
    |ctx: TestContext| async move {
...
test_in_memory!(test_compute_cost_bounds, |ctx: TestContext| async move {
    ...
    println!(
        "Attack scenario: {} CU ({:.1}% of limit)",
        attack_cu, attack_pct
    );
    println!(
        "With limit: {} CU ({:.1}% of limit)",
        limited_cu, limited_pct
    );
