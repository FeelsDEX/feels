...
async fn test_feelssol_must_be_token_0() {
    ...
    assert!(err.to_string().contains("Invalid token order") || err.to_string().contains("FeelsSOL must be token_0"), 
            "Expected token order error, got: {}", err);
}
...
async fn test_correct_token_ordering() {
    ...
    let result = sdk::sort_tokens_with_feelssol(
        ctx.feelssol_mint,
        token_mint.pubkey(),
        ctx.feelssol_mint,
    );
    ...
    match result {
        Ok((token_0, token_1)) => {
            ...
            assert_eq!(token_1, token_mint.pubkey(), "Other token should be token_1");
