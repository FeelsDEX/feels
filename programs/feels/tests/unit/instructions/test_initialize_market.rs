...
use feels::{
    error::FeelsError,
    instructions::{InitializeMarket, InitializeMarketParams},
    state::{Market, Buffer, OracleState, PolicyV1, TokenType, TokenOrigin},
    constants::*,
    utils::token_ordering::*,
};
...
fn test_initialize_market_validation() {
    ...
    let params = InitializeMarketParams {
        ...
        feelssol_is_token_0: true,
    };
    
    assert_eq!(
        validate_market_params(&params, &feelssol_mint, &other_mint).unwrap_err(),
    ...
    assert_eq!(
        validate_market_params(&params, &feelssol_mint, &other_mint).unwrap_err(),
        FeelsError::InvalidSqrtPrice
    );
    
    // Test token ordering violation
    let params = InitializeMarketParams {
        tick_spacing: 10,
        initial_sqrt_price: 1 << 64,
        feelssol_is_token_0: false, // Should be true based on pubkey ordering
    };
...
fn test_market_tick_spacing_validation() {
    ...
    for spacing in valid_spacings {
        ...
        assert!(validate_market_params(&params, &feelssol_mint, &other_mint).is_ok());
    }
    ...
    for spacing in invalid_spacings {
        ...
        assert!(validate_market_params(&params, &feelssol_mint, &other_mint).is_err());
    }
}
...
fn test_oracle_state_initialization() {
    ...
    let oracle = OracleState {
        ...
        initialized: true,
        observation_index: 0,
        oldest_observation_index: 0,
        cardinality: INITIAL_ORACLE_CARDINALITY,
        cardinality_next: INITIAL_ORACLE_CARDINALITY,
        observation_count: 0,
        observations: vec![Default::default(); MAX_ORACLE_OBSERVATIONS],
        _reserved: [0; 64],
    };
    
    // Verify oracle initialization
    assert!(oracle.initialized);
    assert_eq!(oracle.market, market_key);
    assert_eq!(oracle.observation_index, 0);
    assert_eq!(oracle.cardinality, INITIAL_ORACLE_CARDINALITY);
...
fn test_market_pda_derivation() {
    ...
    let (authority_pda, authority_bump) = Pubkey::find_program_address(
        &[b"market_authority", market_pda.as_ref()],
        &program_id
    );
...
fn validate_market_params(
    ...
    // Validate sqrt price
    if params.initial_sqrt_price < MIN_SQRT_PRICE || params.initial_sqrt_price > MAX_SQRT_PRICE {
        return Err(FeelsError::InvalidSqrtPrice.into());
    }
    
    // Validate token ordering (FeelsSOL must be token_0)
    let should_feelssol_be_token_0 = feelssol_mint < other_mint;
    if params.feelssol_is_token_0 != should_feelssol_be_token_0 {
        return Err(FeelsError::InvalidTokenOrder.into());
    }
