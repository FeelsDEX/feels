...
fn test_close_position_amounts_calculation() {
    ...
    let (amount_0, amount_1) =
        calculate_position_amounts(&position_in_range, market.sqrt_price, market.current_tick)
            .unwrap();
    ...
    let (amount_0, amount_1) =
        calculate_position_amounts(&position_below, market.sqrt_price, market.current_tick)
            .unwrap();
    ...
    let (amount_0, amount_1) =
        calculate_position_amounts(&position_above, market.sqrt_price, market.current_tick)
            .unwrap();
...
fn test_close_position_minimum_amounts() {
    ...
    assert!(validate_minimum_amounts(amount_0, amount_1, min_amount_0, min_amount_1).is_ok());

    // Test with insufficient amount_0
    let min_amount_0 = 1100;
    assert_eq!(
        validate_minimum_amounts(amount_0, amount_1, min_amount_0, min_amount_1).unwrap_err(),
