...
mod test_tick_math_edge_cases {
    ...
    fn test_boundary_tick_values() {
        ...
        assert_eq!(sqrt_price_at_min, MIN_SQRT_PRICE_X96, "MIN_TICK should map to MIN_SQRT_PRICE_X96");
        
        let sqrt_price_at_max = TickMath::get_sqrt_ratio_at_tick(MAX_TICK).unwrap();
        assert_eq!(sqrt_price_at_max, MAX_SQRT_PRICE_X96, "MAX_TICK should map to MAX_SQRT_PRICE_X96");
    ...
    fn test_precision_at_extreme_ticks() {
        ...
        for tick in extreme_negative_ticks {
            ...
            assert!(error <= 1, "Extreme negative tick {} has error {}", tick, error);
        }
        ...
        for tick in extreme_positive_ticks {
            ...
            let error = (recovered_tick - tick).abs();
            assert!(error <= 1, "Extreme positive tick {} has error {}", tick, error);
        }
    }
    ...
    fn test_malicious_sqrt_price_inputs() {
        ...
        for input in malicious_inputs {
            if input >= MIN_SQRT_PRICE_X96 && input <= MAX_SQRT_PRICE_X96 {
                ...
                assert!(tick >= MIN_TICK && tick <= MAX_TICK, 
                    "Tick {} out of valid range for input {}", tick, input);
            }
        }
    }
        ...
        for spacing in spacings {
            ...
            for i in -10..=10 {
                ...
                if tick >= MIN_TICK && tick <= MAX_TICK {
                    ...
                    assert!(error <= 1, "Tick {} with spacing {} has error {}", tick, spacing, error);
                }
            }
        }
    }
    ...
    fn test_negative_tick_reciprocal_constants() {
        ...
        for i in 0..19 {
            ...
            if tick >= MIN_TICK {
                ...
                assert!(sqrt_price <= u128::MAX, "Negative tick {} overflowed", tick);
                
                // Verify the price is reasonable (should be < 1.0 for negative ticks)
                let one_in_q96 = 1u128 << 96; // 1.0 in Q96 format
                assert!(sqrt_price < one_in_q96, "Negative tick {} should produce price < 1.0", tick);
            }
        }
    }
    ...
    fn test_ilog2_precision() {
        ...
        for val in test_values {
            ...
            let upper_bound = if log2_val < 127 { 1u128 << (log2_val + 1) } else { u128::MAX };
            
            assert!(val >= lower_bound && val < upper_bound,
                "ilog2 precision error for {}: log2={}", val, log2_val);
        }
    }
        ...
        for tick in (MIN_TICK..=MAX_TICK).step_by(1000) {
            ...
            if last_price > 0 {
                assert!(price > last_price, 
                    "Price not monotonic at tick {}: {} <= {}", tick, price, last_price);
            }
    ...
    fn test_price_to_tick_inverse_consistency() {
        ...
        for price in test_prices {
            ...
            assert!(recovered_price <= price, 
                "Recovered price {} > input price {} for tick {}", 
                recovered_price, price, tick);
            ...
            if tick < MAX_TICK {
                ...
                assert!(next_price > price || tick == MAX_TICK - 1,
    ...
    fn test_attack_vector_price_manipulation() {
        ...
        assert!(result.is_ok());
        let tick = result.unwrap();
        assert!(tick >= MIN_TICK && tick <= MIN_TICK + 1);
        ...
        for val in tricky_values {
            if val >= MIN_SQRT_PRICE_X96 && val <= MAX_SQRT_PRICE_X96 {
