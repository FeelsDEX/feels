...
test_in_memory!(test_ceiling_division, |ctx: TestContext| async move {
    // Test exact division
...
test_in_memory!(test_calculate_fee_ceil, |ctx: TestContext| async move {
...
test_in_memory!(test_attack_prevention, |ctx: TestContext| async move {
    ...
    for amount in attack_amounts {
        ...
        assert!(
            fee >= min_acceptable_fee,
            "Fee {} for amount {} is too low",
            fee,
            amount
        );
    }
