...
test_in_memory!(test_pomm_remove_liquidity_validation, |ctx: TestContext| async move {
...
test_in_memory!(test_pomm_rebalance_validation, |ctx: TestContext| async move {
    ...
    assert!(
        is_valid_range,
        "Should accept valid tick range"
    );
...
test_in_memory!(test_pomm_buffer_accounting, |ctx: TestContext| async move {
    ...
    assert_eq!(new_tau, 4_250_000u128, "Tau should increase by total returned");
...
test_in_memory!(test_pomm_market_liquidity_updates, |ctx: TestContext| async move {
