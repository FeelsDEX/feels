...
impl GeyserE2ETest {
    ...
    async fn start_validator_with_geyser(&mut self) -> TestResult<()> {
        ...
        let project_root =
            project_root.ok_or("Could not find project root with start-geyser-devnet.sh")?;
    ...
    async fn deploy_feels_protocol(&self) -> TestResult<()> {
        ...
        let project_root = project_root.ok_or("Could not find project root with Anchor.toml")?;
    ...
    async fn start_indexer(&mut self) -> TestResult<()> {
        ...
        let project_root =
            project_root.ok_or("Could not find project root with start-indexer.sh")?;
    ...
    async fn validate_indexer_events(&self, expected_txs: &[String]) -> TestResult<()> {
        ...
        for tx_sig in expected_txs {
            let url = format!(
                "{}/api/v1/transactions/{}",
                self.config.indexer_api_url, tx_sig
            );
...
async fn test_complete_geyser_e2e_flow() -> TestResult<()> {
    ...
    e2e_test
        .start_validator_with_geyser()
        .await
        .map_err(|e| format!("Failed to start validator: {}", e))?;

    // Step 2: Deploy Feels Protocol
    e2e_test
        .deploy_feels_protocol()
        .await
    ...
    let ctx = TestContext::new(environment::TestEnvironment::InMemory)
        .await
    ...
    let trader_enter_result = ctx
        .enter_feelssol(&trader, &trader_jitosol, &trader_feelssol, jitosol_amount)
        .await?;
    ...
    let trader_meme_account = ctx
        .create_ata(&trader.pubkey(), &meme_token_mint.pubkey())
        .await?;
    ...
    println!(
        "   FeelsSOL spent: {}",
        trader_feelssol_balance - trader_feelssol_after
    );
    println!("   MEME received: {}", trader_meme_balance);

    // Step 10: Validate indexer captured all events
    e2e_test
        .validate_indexer_events(&transaction_signatures)
        .await?;
...
async fn test_geyser_infrastructure_smoke() -> TestResult<()> {
    ...
    let project_root = project_root.ok_or("Could not find project root with justfile")?;

    // Check if required justfiles and configs exist (new structure)
    let main_justfile = project_root.join("justfile");
    let e2e_justfile = project_root.join("justfiles/e2e.just");

    assert!(
        main_justfile.exists(),
        "Main justfile not found at project root"
    );
    assert!(
        e2e_justfile.exists(),
        "E2E justfile not found at justfiles/e2e.just"
    );
    ...
    assert!(
        indexer_config.exists() || indexer_e2e_config.exists(),
