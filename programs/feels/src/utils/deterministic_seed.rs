...
impl CanonicalSeeds {
    ...
    pub fn derive_pool_pda(
        ...
        let (token_a_sorted, token_b_sorted) = Self::sort_token_mints(mint_a, mint_b);
    ...
    pub fn derive_tick_array_pda(
        ...
        Pubkey::find_program_address(
            &[b"tick_array", pool.as_ref(), &start_tick.to_le_bytes()],
            program_id,
        )
    }

    /// Get tick array seeds for PDA signing
    pub fn get_tick_array_seeds(pool: &Pubkey, start_tick: i32, bump: u8) -> Vec<Vec<u8>> {
        vec![
            b"tick_array".to_vec(),
            pool.as_ref().to_vec(),
...
// ErrorCode enum removed - using consolidated FeelsProtocolError
...
mod tests {
    ...
    fn test_canonical_ordering() {
        ...
        let (token_a_ab, token_b_ab) = CanonicalSeeds::sort_token_mints(&mint_a, &mint_b);
        let (token_a_ba, token_b_ba) = CanonicalSeeds::sort_token_mints(&mint_b, &mint_a);

        assert_eq!(token_a_ab, token_a_ba);
        assert_eq!(token_b_ab, token_b_ba);
        assert_eq!(token_a_ab, mint_a);
        assert_eq!(token_b_ab, mint_b);
    }
    ...
    fn test_pda_consistency() {
        ...
        let (pda_ab, bump_ab) =
            CanonicalSeeds::derive_pool_pda(&mint_a, &mint_b, fee_rate, &program_id);
        let (pda_ba, bump_ba) =
            CanonicalSeeds::derive_pool_pda(&mint_b, &mint_a, fee_rate, &program_id);
