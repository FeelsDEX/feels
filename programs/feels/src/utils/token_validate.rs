...
pub fn validate_ticker_not_restricted(ticker: &str) -> Result<()> {
    if is_ticker_restricted(ticker) {
        return Err(error!(crate::state::PoolError::RestrictedTicker));
    }
    Ok(())
}
...
pub fn validate_ticker_format(ticker: &str) -> Result<()> {
    // Check length
    if ticker.is_empty() {
        return Err(error!(crate::state::PoolError::InvalidTickerLength));
    }
    ...
    if !ticker.chars().all(|c| c.is_ascii_alphanumeric()) {
        return Err(error!(crate::state::PoolError::InvalidTickerFormat));
    }
