...
pub use spl_math::precise_number::PreciseNumber;
...
pub mod big_int {
    ...
    pub fn mul_div(a: U256, numerator: U256, denominator: U256, rounding: Rounding) -> Option<U256> {
...
pub mod safe {
    ...
    pub fn add_u64(a: u64, b: u64) -> Result<u64> {
        a.checked_add(b).ok_or(PoolError::MathOverflow.into())
    }

    /// Safe subtraction for u64 values
    pub fn sub_u64(a: u64, b: u64) -> Result<u64> {
        a.checked_sub(b).ok_or(PoolError::ArithmeticUnderflow.into())
    }

    /// Safe multiplication for u64 values
    pub fn mul_u64(a: u64, b: u64) -> Result<u64> {
        a.checked_mul(b).ok_or(PoolError::MathOverflow.into())
    }
    ...
    pub fn div_u64(a: u64, b: u64) -> Result<u64> {
        ...
        a.checked_div(b).ok_or(PoolError::DivisionByZero.into())
    }
    ...
    pub fn add_liquidity_delta(liquidity: u128, delta: i128) -> Result<u128> {
        if delta >= 0 {
            liquidity.checked_add(delta as u128).ok_or(PoolError::LiquidityOverflow.into())
        } else {
            liquidity.checked_sub((-delta) as u128).ok_or(PoolError::LiquidityUnderflow.into())
        }
    }
    ...
    pub fn calculate_percentage(amount: u64, basis_points: u16) -> Result<u64> {
        ...
        if basis_points > BASIS_POINTS_DIVISOR as u16 {
            return Err(PoolError::InvalidPercentage.into());
        }
...
pub mod q96 {
    ...
    pub fn calculate_fee_growth_q128(fee_amount: u64, liquidity: u128) -> Result<[u64; 4]> {
        if liquidity == 0 {
            return Err(PoolError::InvalidLiquidity.into());
        }
        ...
        let fee_shifted = fee_u256.checked_shl(128)
            .ok_or(PoolError::MathOverflow)?;
        let result = fee_shifted.checked_div(U256::from(liquidity))
            .ok_or(PoolError::DivisionByZero)?;
...
pub mod amm {
    ...
    use anchor_lang::prelude::*;
    use crate::state::PoolError;
    use crate::constant::{BASIS_POINTS_DENOMINATOR, MAX_PROTOCOL_FEE_RATE, MIN_TICK, MAX_TICK};
    ...
    impl TickMath {
        ...
        pub fn get_tick_at_sqrt_ratio(sqrt_ratio_x96: u128) -> Result<i32> {
            ...
            if tick_low == tick_high {
                ...
                let actual_tick_high_sqrt_price = Self::get_sqrt_ratio_at_tick(tick_high.min(MAX_TICK))?;
    ...
    impl FeeGrowthMath {
        ...
        pub fn add_fee_growth(a: [u64; 4], b: [u64; 4]) -> Result<[u64; 4]> {
            use super::big_int::{words_to_u256, u256_to_words};
            
            let a_u256 = words_to_u256(a);
            let b_u256 = words_to_u256(b);
            
            let result = a_u256.checked_add(b_u256)
                .ok_or(PoolError::MathOverflow)?;
        ...
        pub fn sub_fee_growth(a: [u64; 4], b: [u64; 4]) -> Result<[u64; 4]> {
            ...
            let result = a_u256.checked_sub(b_u256)
    ...
    impl FeeMath {
        ...
        pub fn calculate_swap_fees(amount_in: u64, fee_rate: u16, protocol_fee_rate: u16) -> Result<FeeBreakdown> {
            // Calculate total fee first
            let total_fee = Self::calculate_total_fee(amount_in, fee_rate)?;
            
            // Calculate protocol's share of the fee
            let protocol_fee = mul_div_u64(total_fee, protocol_fee_rate as u64, BASIS_POINTS_DENOMINATOR as u64)?;
        ...
        pub fn calculate_effective_fee_rate(base_fee_rate: u16, _volume: u128, _volatility: u32) -> Result<u16> {
    ...
    impl FeeConfig {
        ...
        pub fn get_tick_spacing_for_fee(fee_rate: u16) -> Result<i16> {
            match fee_rate {
                ...
                _ => Err(PoolError::InvalidFeeRate.into()),
            }
        }
        
        /// Validate pool fee configuration
        pub fn validate_pool_fees(fee_rate: u16, protocol_fee_rate: u16, tick_spacing: i16) -> Result<()> {
    ...
    pub fn get_amount_0_delta(
        ...
        let (quotient, remainder) = (
            numerator.checked_div(denominator).ok_or(PoolError::DivisionByZero)?,
            numerator.checked_rem(denominator).ok_or(PoolError::DivisionByZero)?
        );
        ...
        result.try_into().map_err(|_| PoolError::MathOverflow.into())
    }
    ...
    pub fn get_liquidity_for_amount_1(
        ...
        let numerator = U256::from(amount_1).checked_shl(96).ok_or(PoolError::MathOverflow)?;
    ...
    /// 
    /// # Returns
    /// The new sqrt price after the swap, in Q96 format
    pub fn get_next_sqrt_price_from_amount_0_rounding_up(
        ...
        let product = U256::from(sqrt_price_x64).checked_mul(U256::from(amount_u64 as u128))
            .ok_or(PoolError::MathOverflow)?;
        ...
        let liquidity_shift_left = U256::from(liquidity).checked_shl(64).ok_or(PoolError::MathOverflow)?;
        if !add && liquidity_shift_left <= product {
            return Err(PoolError::DivisionByZero.into());
        }
        
        let denominator = if add {
            liquidity_shift_left.checked_add(product).ok_or(PoolError::MathOverflow)?
        } else {
            liquidity_shift_left.checked_sub(product).ok_or(PoolError::ArithmeticUnderflow)?
        };
        
        let price_x64 = mul_div_rounding_up(numerator, U256::ONE, denominator)
            .ok_or(PoolError::MathOverflow)?;
        
        let price_x64_u128: u128 = price_x64.try_into().map_err(|_| PoolError::MathOverflow)?;
        
        // Convert back from Q64 to Q96
        let price_x96 = q64_to_q96(price_x64_u128);
        
        if !(MIN_SQRT_PRICE_X96..=MAX_SQRT_PRICE_X96).contains(&price_x96) {
            Err(PoolError::PriceOutOfBounds.into())
        } else {
            Ok(price_x96)
        }
    }
    ...
    mod tests {
        ...
        fn test_tick_to_sqrt_price_consistency() {
            ...
            for &tick in &test_ticks {
                ...
                assert!(sqrt_price_x96 >= MIN_SQRT_PRICE_X96, 
                    "sqrt_price {} < MIN {} for tick {}", sqrt_price_x96, MIN_SQRT_PRICE_X96, tick);
                assert!(sqrt_price_x96 <= MAX_SQRT_PRICE_X96,
                    "sqrt_price {} > MAX {} for tick {}", sqrt_price_x96, MAX_SQRT_PRICE_X96, tick);
        ...
        fn test_tick_sqrt_price_symmetry() {
            ...
            for tick in test_ticks {
                ...
                assert!(
                    ...
                    tick, sqrt_price, recovered_tick
                );
            }
        }
        ...
        fn test_sqrt_price_monotonicity() {
            ...
            for tick in test_ticks {
                let sqrt_price = TickMath::get_sqrt_ratio_at_tick(tick).unwrap();
                assert!(sqrt_price > last_sqrt_price, 
                    "Sqrt price not monotonic at tick {}", tick);
