...
pub fn update_floor(ctx: Context<UpdateFloor>) -> Result<()> {
    ...
    for account_info in ctx.remaining_accounts {
        // Prevent double counting the same account
        require!(
            seen_accounts.insert(account_info.key()),
        ...
        // Only count protocol-held inventory (must be owned by buffer authority)
        require!(
            token_account.owner == buffer.authority,
