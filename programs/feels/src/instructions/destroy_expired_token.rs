...
use crate::{
    constants::{ESCROW_SEED, ESCROW_AUTHORITY_SEED, MARKET_SEED, PROTOCOL_TOKEN_SEED},
    error::FeelsError,
    events::TokenDestroyed,
    state::{PreLaunchEscrow, Market, ProtocolToken, ProtocolConfig},
};
...
pub fn destroy_expired_token(ctx: Context<DestroyExpiredToken>) -> Result<()> {
    ...
    let expiration_time = protocol_token.created_at
    ...
    if let Some(market_info) = &ctx.accounts.market {
        ...
        let (expected_market, _bump) = Pubkey::find_program_address(
            &[MARKET_SEED, ctx.accounts.token_mint.key().as_ref(), ctx.accounts.escrow.feelssol_mint.as_ref()],
            ctx.program_id,
        );
