...
pub fn initialize_protocol(
    ...
    require!(params.depeg_threshold_bps >= 25 && params.depeg_threshold_bps <= 5000, FeelsError::InvalidMarket);
    require!(params.depeg_required_obs > 0 && params.depeg_required_obs <= 10, FeelsError::InvalidMarket);
    require!(params.clear_required_obs > 0 && params.clear_required_obs <= 10, FeelsError::InvalidMarket);
    require!(params.dex_twap_window_secs >= 300 && params.dex_twap_window_secs <= 7200, FeelsError::InvalidMarket);
    require!(params.dex_twap_stale_age_secs >= params.dex_twap_window_secs, FeelsError::InvalidMarket);
    ...
    let mut i = 0usize;
    for k in params.dex_whitelist.iter().take(8) { config.dex_whitelist[i] = *k; i+=1; }
...
pub fn update_protocol(
    ...
    if let Some(k) = params.dex_twap_updater { config.dex_twap_updater = k; }
    if let Some(x) = params.depeg_threshold_bps { require!((25..=5000).contains(&x), FeelsError::InvalidMarket); config.depeg_threshold_bps = x; }
    if let Some(x) = params.depeg_required_obs { require!(x > 0 && x <= 10, FeelsError::InvalidMarket); config.depeg_required_obs = x; }
    if let Some(x) = params.clear_required_obs { require!(x > 0 && x <= 10, FeelsError::InvalidMarket); config.clear_required_obs = x; }
    if let Some(x) = params.dex_twap_window_secs { require!((300..=7200).contains(&x), FeelsError::InvalidMarket); config.dex_twap_window_secs = x; }
    if let Some(x) = params.dex_twap_stale_age_secs { require!(x >= config.dex_twap_window_secs, FeelsError::InvalidMarket); config.dex_twap_stale_age_secs = x; }
    if let Some(list) = params.dex_whitelist.as_ref() {
        config.dex_whitelist = [Pubkey::default(); 8];
        let mut i = 0usize;
        for k in list.iter().take(8) { config.dex_whitelist[i] = *k; i+=1; }
        config.dex_whitelist_len = i as u8;
    }
    if let Some(x) = params.mint_per_slot_cap_feelssol { config.mint_per_slot_cap_feelssol = x; }
    if let Some(x) = params.redeem_per_slot_cap_feelssol { config.redeem_per_slot_cap_feelssol = x; }
