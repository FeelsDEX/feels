...
//! 
//! SECURITY: This instruction updates fee accrual for the upper tick only.
//! Combined with update_position_fee_lower, it allows fee collection for 
...
pub fn update_position_fee_upper(ctx: Context<UpdatePositionFeeUpper>) -> Result<()> {
    ...
    let fee_growth_above_0 = if market.current_tick < position.tick_upper {
        ...
        market.fee_growth_global_0_x64.wrapping_sub(upper_tick.fee_growth_outside_0_x64)
    };
    
    let fee_growth_above_1 = if market.current_tick < position.tick_upper {
        upper_tick.fee_growth_outside_1_x64
    } else {
        market.fee_growth_global_1_x64.wrapping_sub(upper_tick.fee_growth_outside_1_x64)
    };
    ...
    let fee_growth_inside_0 = market.fee_growth_global_0_x64
        .wrapping_sub(fee_growth_above_0);
    
    let fee_growth_inside_1 = market.fee_growth_global_1_x64
        .wrapping_sub(fee_growth_above_1);
    
    // Calculate fees owed
    let fee_growth_delta_0 = fee_growth_inside_0
        .wrapping_sub(position.fee_growth_inside_0_last_x64);
