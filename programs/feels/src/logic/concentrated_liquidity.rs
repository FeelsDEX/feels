...
impl ConcentratedLiquidityMath {
    ...
    pub fn get_amounts_for_concentrated_liquidity(
        ...
        if sqrt_price_current <= sqrt_price_lower {
            // All token A
            amount_a = get_amount_0_delta(sqrt_price_lower, sqrt_price_upper, concentrated_liquidity, true)?;
            amount_b = 0;
        } else if sqrt_price_current < sqrt_price_upper {
            // Both tokens
            amount_a = get_amount_0_delta(sqrt_price_current, sqrt_price_upper, concentrated_liquidity, true)?;
            amount_b = get_amount_1_delta(sqrt_price_lower, sqrt_price_current, concentrated_liquidity, true)?
        } else {
            // All token B
            amount_a = 0;
            amount_b = get_amount_1_delta(sqrt_price_lower, sqrt_price_upper, concentrated_liquidity, true)?;
        }

        let amount_a_u64 = amount_a.try_into()
            .map_err(|_| PoolError::ArithmeticOverflow)?;
        let amount_b_u64 = amount_b.try_into()
            .map_err(|_| PoolError::ArithmeticOverflow)?;
    ...
    pub fn get_concentrated_liquidity_for_amounts(
        ...
        if sqrt_price_current <= sqrt_price_lower {
            ...
            let liquidity_0 = get_liquidity_for_amount_0(sqrt_price_current, sqrt_price_upper, amount_0 as u128)?;
            let liquidity_1 = get_liquidity_for_amount_1(sqrt_price_lower, sqrt_price_current, amount_1 as u128)?;
    ...
    pub fn get_next_sqrt_price_from_input(
        ...
        require!(
            (MIN_SQRT_PRICE_X96..=MAX_SQRT_PRICE_X96).contains(&sqrt_price),
        ...
        if zero_for_one {
            get_next_sqrt_price_from_amount_0_rounding_up(sqrt_price, liquidity, amount_in as u128, true)
        } else {
            get_next_sqrt_price_from_amount_1_rounding_down(sqrt_price, liquidity, amount_in as u128, true)
        }
    }
    ...
    pub fn get_next_sqrt_price_from_output(
        ...
        if zero_for_one {
            get_next_sqrt_price_from_amount_1_rounding_down(sqrt_price, liquidity, amount_out as u128, false)
        } else {
            get_next_sqrt_price_from_amount_0_rounding_up(sqrt_price, liquidity, amount_out as u128, false)
        }
    }
