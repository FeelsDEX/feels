...
use crate::events::{CircuitBreakerActivated, RedemptionsPaused, RedemptionsResumed, SafetyPaused, SafetyResumed};
...
impl SafetyController {
    ...
    pub fn check_and_update_divergence(
        ...
        if !dex_stale && oracle.native_rate_q64 > 0 && oracle.dex_twap_rate_q64 > 0 {
            ...
            if div_bps >= config.depeg_threshold_bps {
                ...
                if !self.redemptions_paused && self.consecutive_breaches >= config.depeg_required_obs {
    ...
    pub fn check_redemptions_allowed(&self, oracle: &ProtocolOracle, config: &ProtocolConfig, current_ts: i64) -> Result<()> {
        // First check if already paused
        if self.redemptions_paused {
            return Err(crate::error::FeelsError::MarketPaused.into());
        }
        
        // Optionally emit a warning if oracle is stale (but don't modify state)
        let dex_stale = if oracle.dex_last_update_ts > 0 {
            (current_ts - oracle.dex_last_update_ts) > config.dex_twap_stale_age_secs as i64
        } else {
            true
        ...
        if dex_stale {
            msg!("Warning: DEX TWAP oracle is stale - redemptions allowed but oracle needs update");
        }
    ...
    pub fn should_restrict_large_trades(&self) -> bool {
        self.degrade_flags.low_liquidity || 
        self.degrade_flags.high_volatility
    }
}
