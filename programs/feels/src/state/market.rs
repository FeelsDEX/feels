...
use crate::state::{TokenType, TokenOrigin};
...
impl Market {
    ...
    pub fn derive_vault_address_with_key(&self, market_key: &Pubkey, mint: &Pubkey, program_id: &Pubkey) -> (Pubkey, u8) {
        crate::utils::derive_vault(market_key, mint, program_id)
    }
    
    /// Static method to derive vault address without Market instance
    pub fn derive_vault_address(market_key: &Pubkey, mint: &Pubkey, program_id: &Pubkey) -> (Pubkey, u8) {
        crate::utils::derive_vault(market_key, mint, program_id)
    }
    
    /// Get the derived vault addresses for token_0 and token_1
    /// Note: market_key must be passed in since self doesn't have access to its own key
    pub fn get_vault_addresses(&self, market_key: &Pubkey, program_id: &Pubkey) -> ((Pubkey, u8), (Pubkey, u8)) {
    ...
    /// Note: market_key must be passed in since self doesn't have access to its own key
    pub fn derive_market_authority_with_key(&self, market_key: &Pubkey, program_id: &Pubkey) -> (Pubkey, u8) {
